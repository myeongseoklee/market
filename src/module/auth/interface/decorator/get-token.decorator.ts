 ...
 * @returns {Member} member - 요청 객체에서 추출된 사용자 정보
 */

export const GetToken = createParamDecorator(
  (data: unknown, ctx: ExecutionContext): { accessToken: string; refreshToken: string } => {
    const req = ctx.switchToHttp().getRequest();
    return req.user as { accessToken: string; refreshToken: string; };
  },
);
